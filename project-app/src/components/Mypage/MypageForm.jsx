...
const UserSection = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        alert('회원 탈퇴 실패: ' + (res.data.message || '알 수 없음'))
      }
    } catch {
      alert('서버 오류가 발생했습니다.')
    }
  ...
  const searchAddress = () => {
    new window.daum.Postcode({
      oncomplete: (data) => setForm(f => ({ ...f, address: data.roadAddress }))
    }).open()
  }
  ...
  return (
    <section style={{ display: 'flex', justifyContent: 'center', marginBottom: '40px' }}>
      <div style={{
        width: '100%', maxWidth: '1000px', padding: '32px 40px',
        background: '#fff', border: '1px solid #d1d5db', borderRadius: '16px',
        boxShadow: '0 4px 6px rgba(0,0,0,.05), 0 10px 20px rgba(0,0,0,.08)'
      }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={() => { setForm({ ...info }); setEditOpen(true) }}
            style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid #d1d5db', background: '#fff', cursor: 'pointer', fontSize: '14px' }}>
            정보 수정
          </button>
          <button onClick={handleDelete}
            style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid #fca5a5', background: '#fff', color: '#ef4444', cursor: 'pointer', fontSize: '14px' }}>
            회원 탈퇴
          </button>
        </div>
      </div>
      ...
      {editOpen && (
        <div onClick={() => setEditOpen(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            width: '100%', maxWidth: '480px', boxShadow: '0 8px 20px rgba(0,0,0,0.1)'
          }}>
              ...
              <div key={key} style={{ marginBottom: '12px' }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    background: readOnly ? '#f9fafb' : '#fff', color: readOnly ? '#9ca3af' : '#111827'
                  }}
                />
              </div>
            ))}
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <input value={form.address || ''} readOnly
                  style={{ flex: 1, padding: '8px 12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '14px', background: '#f9fafb', color: '#9ca3af' }} />
                <button onClick={searchAddress}
                  style={{ padding: '8px 14px', border: '1px solid #d1d5db', borderRadius: '8px', background: '#fff', cursor: 'pointer', fontSize: '14px', whiteSpace: 'nowrap' }}>
                  주소 검색
                </button>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', borderTop: '1px solid #e5e7eb', paddingTop: '12px' }}>
              <button onClick={() => setEditOpen(false)}
                style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#e5e7eb', cursor: 'pointer', fontSize: '14px' }}>
                취소
              </button>
              <button onClick={handleUpdate}
                style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#3b82f6', color: '#fff', cursor: 'pointer', fontSize: '14px' }}>
                수정
              </button>
            </div>
          </div>
...
const PetModal = ({ title, form, setForm, onSubmit, onClose, onDelete, submitLabel, preview, setPreview, fileRef }) => {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: '#fff', borderRadius: '12px', padding: '24px',
        width: '100%', maxWidth: '600px', boxShadow: '0 8px 20px rgba(0,0,0,0.1)',
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '12px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
            <div onClick={() => fileRef.current?.click()} style={{
              width: '150px', height: '150px', border: '2px dashed #a0aec0',
              ...
              {preview
                ...
                : <span style={{ fontSize: '12px', color: '#a0aec0', textAlign: 'center' }}>클릭하여<br />이미지 선택</span>
              }
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
          ...
          <div>
            ...
            <select value={form.size} onChange={e => setForm(f => ({ ...f, size: e.target.value }))} style={selectStyle}>
              {['소형', '중형', '대형'].map(o => <option key={o} value={o}>{o}</option>)}
            </select>
          </div>
          <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
          <div>
            ...
            <select value={form.gender} onChange={e => setForm(f => ({ ...f, gender: e.target.value }))} style={selectStyle}>
              {['수컷', '암컷'].map(o => <option key={o} value={o}>{o}</option>)}
            </select>
          </div>
          <div>
            <label style={labelStyle}>중성화 여부</label>
            <select value={form.neutered} onChange={e => setForm(f => ({ ...f, neutered: e.target.value }))} style={selectStyle}>
              {['예', '아니오'].map(o => <option key={o} value={o}>{o}</option>)}
            </select>
          </div>
          <div>
            <label style={labelStyle}>예방접종 여부</label>
            <select value={form.vaccination} onChange={e => setForm(f => ({ ...f, vaccination: e.target.value }))} style={selectStyle}>
              {['예', '아니오'].map(o => <option key={o} value={o}>{o}</option>)}
            </select>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '12px' }}>
          <label style={labelStyle}>건강 증명서</label>
          <input type="file" name="certificateFile" style={{ width: '100%', fontSize: '14px' }} />
        </div>

        <div style={{ marginBottom: '16px' }}>
          <label style={labelStyle}>기타 사항</label>
          <textarea value={form.etc} onChange={e => setForm(f => ({ ...f, etc: e.target.value }))}
...
const PetSection = () => {
  ...
  const handleAdd = async () => {
    try {
      await axios.post('/api/pets/add', buildFormData(), { headers: getAuthHeader() })
      alert('반려견이 등록되었습니다.')
      setAddOpen(false)
      loadPets()
    } catch {
  ...
  const handleEdit = async () => {
    ...
    fd.append('petNo', editPetNo)
    try {
      await axios.post('/api/pets/update', fd, { headers: getAuthHeader() })
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/api/pets/delete/${editPetNo}`, { headers: getAuthHeader() })
      alert('반려견이 삭제되었습니다.')
      setEditOpen(false)
      loadPets()
    } catch {
    ...
    <section style={{ display: 'flex', justifyContent: 'center', marginBottom: '40px' }}>
      <div style={{
        ...
        {pets.length === 0
          ...
          : pets.map(pet => (
            <div key={pet.no} style={{
              display: 'flex', alignItems: 'center', padding: '24px',
              border: '1px solid #d1d5db', borderRadius: '16px', marginBottom: '12px',
              boxShadow: '0 4px 6px rgba(0,0,0,.05)'
            }}>
              <div style={{ marginRight: '30px' }}>
                <img src={`http://localhost:8080/api/pets/image/${pet.no}`} alt={pet.name}
              ...
              <button onClick={() => openEdit(pet.no)}
                style={{ marginLeft: 'auto', padding: '6px 14px', borderRadius: '8px', border: '1px solid #d1d5db', background: '#fff', cursor: 'pointer', fontSize: '14px' }}>
                수정하기
              </button>
            </div>
          ))
        ...
        <button onClick={openAdd} style={{
          width: '100%', padding: '20px', border: '2px dashed #6b7280', borderRadius: '16px',
          background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center',
          justifyContent: 'center', gap: '8px', fontSize: '16px', fontWeight: 600,
          color: '#6b7280', opacity: 0.6, marginTop: '8px'
        }}>
      ...
      {addOpen && (
        <PetModal title="반려견 추가" form={form} setForm={setForm}
          onSubmit={handleAdd} onClose={() => setAddOpen(false)}
          submitLabel="추가" preview={preview} setPreview={setPreview} fileRef={fileRef} />
      )}
      {editOpen && (
        <PetModal title="반려견 정보 수정" form={form} setForm={setForm}
          onSubmit={handleEdit} onClose={() => setEditOpen(false)}
          onDelete={handleDelete} submitLabel="수정 완료"
...
const EditReservationModal = ({ resNo, onClose, onRefresh }) => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setSelectedSvcIds((d.serviceIds || []).map(String))
  ...
  const toggleService = (id) => {
    const sid = String(id)
    setSelectedSvcIds(prev => prev.includes(sid) ? prev.filter(s => s !== sid) : [...prev, sid])
  }
  ...
  const handleUpdate = async () => {
    ...
    fd.append('total', nights)
    fd.append('totalPrice', total)
    fd.append('roomNo', detail.roomNo)
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '12px', padding: '24px', width: '100%', maxWidth: '560px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 20px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ marginBottom: '12px' }}>
          ...
          {allServices.map(s => {
            ...
            return (
              <label key={s.serviceNo} style={{
                display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px',
                border: `1px solid ${checked ? '#93c5fd' : '#e5e7eb'}`, borderRadius: '8px',
                marginBottom: '6px', cursor: 'pointer', background: checked ? '#eff6ff' : '#fff', fontSize: '14px'
              }}>
                  ...
                  {reservations.map(res => (
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 16px' }} onClick={e => e.stopPropagation()}>
                        {res.status === '예약중' && (
                          <button onClick={() => setEditResNo(res.resNo)}
                            style={{ padding: '5px 12px', borderRadius: '6px', border: '1px solid #d1d5db', background: '#fff', cursor: 'pointer', fontSize: '13px' }}>수정</button>
                        )}
                      </td>
                      <td style={{ padding: '12px 16px' }} onClick={e => e.stopPropagation()}>
                        {res.status === '예약중' && (
                          <button onClick={() => handlePay(res.resNo, res.totalPrice)}
                            style={{ padding: '5px 12px', borderRadius: '6px', border: 'none', background: '#f59e0b', color: '#fff', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>결제</button>
                        )}
                        {res.status === '결제완료' && (
                          <button onClick={() => handleRefund(res.resNo, res.totalPrice)}
    ...
    <div style={{ backgroundColor: '#f5f5f5', minHeight: '100vh', fontFamily: "'Noto Sans KR', system-ui, sans-serif" }}>
      <section style={{ display: 'flex', justifyContent: 'center', margin: '40px 0' }}>
        <div style={{
          width: '100%', maxWidth: '1000px', padding: '32px 40px',
          background: '#fff', border: '1px solid #d1d5db', borderRadius: '16px',
          boxShadow: '0 4px 6px rgba(0,0,0,.05), 0 10px 20px rgba(0,0,0,.08)', textAlign: 'center'
        }}>
